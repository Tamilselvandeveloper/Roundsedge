...
var dburl = "mongodb://localhost:27017/Roundsedge";
...
router.post('/edit', function(req, res, next) {
  MongoClient.connect(dburl, function(err, db) {
    if(err) { throw err;  }
    var collection = db.collection('products');
    var product = {'UserName': req.body.product_name, 'Mobile': req.body.price, 'Address': req.body.category};
    collection.update({'_id':new mongodb.ObjectID(req.body.id)}, {$set:{'UserName': req.body.product_name, 'Mobile': req.body.price, 'Address': req.body.category}}, function(err, result) {
    if(err) { throw err; }
      db.close();
      res.redirect('/');
     });
